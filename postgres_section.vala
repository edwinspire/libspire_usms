...
namespace edwinspire.uSMS {
	public class PostgreSQLConnection:PostgreSqldb {
		//public string old_ts = "2000-01-01";
		//public int old_rows = 0;
	...
	public class AttachmentsTable:PostgreSQLConnection {
		public int insert(string name_file, string md5) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_attachments_insert_row($1::text, $2::text) AS idattach;",  ValuesArray);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var filas in this.Result_FieldName(ref Resultado)) {
						Retorno = filas["idattach"].as_int();
					}
				} else {
	...
	public class GroupsTable:PostgreSQLConnection {
		public string fun_groups_remove_selected_xml(string idgroups, bool fieldtextasbase64 = true) {
			string Retorno = "";
			string[] ValuesArray = {
				"{"+idgroups+"}", fieldtextasbase64.to_string()
			}
			;
			//GLib.print("Llega hasta aqui 3 \n");
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_groups_remove_selected_xml($1::integer[], $2::boolean) AS return;",  ValuesArray);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var filas in this.Result_FieldName(ref Resultado)) {
		...
		public string fun_view_idgroup_name_xml(bool fieldtextasbase64 = true) {
			...
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_view_idgroup_name_xml($1::boolean) as return;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
		...
		public string fun_view_groups_xml(bool fieldtextasbase64 = true) {
			...
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_view_groups_xml($1::boolean) as return;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var reg in this.Result_FieldName(ref Resultado)) {
						RetornoX = reg["return"].Value;
					}
		...
		public string fun_groups_edit_xml(int idgroup, bool enable, string name, string note, string ts, bool fieldtextasbase64 = true) {
			string[] valuesin = {
				idgroup.to_string(), enable.to_string(), name, note, ts, fieldtextasbase64.to_string()
			}
			;
			string RetornoX = "<table></table>";
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_groups_edit_xml($1::integer, $2::boolean, $3::text, $4::text, $5::timestamp without time zone, $6::boolean) as return;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var reg in this.Result_FieldName(ref Resultado)) {
						RetornoX = reg["return"].Value;
					}
	...
	public class TableChangedTable:PostgreSQLConnection {
		public HashMap<string, string> old_ts = new HashMap<string, string>();
		public HashMap<string, bool> status = new HashMap<string, bool>();
		public bool is_changed() {
			bool RetornoX = false;
			string new_ts_ = "2000-01-01";
			string old_ts_ = "2000-01-01";
			string tname_ = "-";
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
				...
				;
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM tables_changed;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var reg in this.Result_FieldName(ref Resultado)) {
						new_ts_ = reg["tts"].Value;
						tname_ = reg["tname"].Value;
						if(old_ts.has_key(tname_)) {
							old_ts_ = old_ts[tname_];
							if(new_ts_ != old_ts_) {
								RetornoX = true;
								status[tname_] = true;
								old_ts[tname_] = new_ts_;
							} else {
								status[tname_] = false;
							}
						} else {
						...
						//    stderr.printf ("%s >> %s >> %s\n", old_ts_, new_ts_, status[tname_].to_string());
					}
				} else {
					stderr.printf ("FETCH ALL failed: %s", Conexion.get_error_message ());
				}
	...
	public class NotificationsTable:PostgresuSMS {
		public string fun_view_notifications_xml(int lastidnotify, bool fieldtextasbase64 = true) {
			string Retorno = "<table></table>";
			string[] ValuesArray = {
				lastidnotify.to_string(), fieldtextasbase64.to_string()
			}
			;
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_view_notification_area_xml($1::INTEGER, $2::BOOLEAN) AS return;",  ValuesArray);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var filas in this.Result_FieldName(ref Resultado)) {
						Retorno = filas["return"].Value;
					}
	...
	public class TableSIM:PostgresuSMS {
		...
		public SIMRow fun_sim_getrow(string phone) {
			string[] valuessms = {
				phone
			}
			;
			...
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_sim_getrow($1::text);",  valuessms);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var fila in this.Result_FieldName(ref Resultado)) {
						Retorno.id = fila["idsim"].as_int();
						Retorno.idprovider = fila["idprovider"].as_int();
	...
	public class PostgresuSMS:PostgreSQLConnection {
		...
		public bool fun_portmodem_update(int inidport, string inport, string incimi, string inimei, string inmanufacturer, string inmodel, string inrevision) {
			string[] valuessms = {
				inidport.to_string(), inport, incimi, inimei, inmanufacturer, inmodel, inrevision
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
		...
		public int fun_currentportsproviders_insertupdate(int inidport, string inport, string incimi, string inimei) {
			string[] valuessms = {
				inidport.to_string(), inport, incimi, inimei
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
	...
	public class TableProvider:PostgresuSMS {
		...
		public string fun_provider_delete_selection_xml(string idproviders, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					"{"+idproviders+"}", fieldtextasbase64.to_string()
				}
				;
	...
	public class SMSOutRow:Submit {
		public edwinspire.PDU.DCS_MESSAGE_CLASS MessageClass {
			get;
			set;
			default = edwinspire.PDU.DCS_MESSAGE_CLASS.TE_SPECIFIC;
		}
		public bool enableMessageClass {
			get;
			set;
			default = false;
		}
		...
		public GLib.DateTime  dateload {
			get;
			set;
			default = new GLib.DateTime.now_local();
		}
		public int  idprovider {
			get;
			set;
		...
		public GLib.DateTime  datetosend {
			set;
			get;
			default = new GLib.DateTime.now_local();
		}
		public GLib.DateTime  dateprocess {
			set;
			get;
			default = new GLib.DateTime.now_local();
		}
		...
		public int  retryonfail {
			get;
			set;
			default = 0;
		}
		public int  maxtimelive {
			get;
			set;
			default = 0;
		}
	...
	public class TableCallIn:PostgresuSMS {
		...
		public int fun_incomingcalls_insert_online(int inidport, OnIncomingCall inOnIncomingCall, string inphone, string innote = "") {
			string[] valuessms = {
				inidport.to_string(), ((int)inOnIncomingCall).to_string(), inphone, innote
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
	...
	public class TableCity:PostgreSQLConnection {
		public string fun_location_city_remove_selected_xml(string ids, bool fieldtextasbase64 = true) {
			string Retorno = "";
			string[] ValuesArray = {
				"{"+ids+"}", fieldtextasbase64.to_string()
			}
			;
			//GLib.print("Llega hasta aqui 3 \n");
			var  Conexion = Postgres.connect_db (this.ConnString());
		...
		public string fun_location_city_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idstate.to_string(), id.to_string(), name, code, ts, fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_view_city_by_idstate_xml(int idcountry, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcountry.to_string(), fieldtextasbase64.to_string()
				}
				;
	...
	public class TableState:PostgreSQLConnection {
		...
		public string fun_location_state_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcountry.to_string(), id.to_string(), name, code, ts, fieldtextasbase64.to_string()
				}
				;
	...
	public class TableSector:PostgreSQLConnection {
		...
		public string fun_location_sector_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcity.to_string(), id.to_string(), name, ts, fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_view_sector_by_idcity_xml(int idcity, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcity.to_string(), fieldtextasbase64.to_string()
				}
				;
	...
	public class TableSubSector:PostgreSQLConnection {
		...
		public string fun_location_subsector_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idsector.to_string(), id.to_string(), name, ts, fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_view_subsector_by_idsector_xml(int idsector, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idsector.to_string(), fieldtextasbase64.to_string()
				}
				;
	...
	public class LocationLevel:PostgreSQLConnection {
		public string fun_location_level_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					level.to_string(), idpk.to_string(), idfk.to_string(), name, code, ts, fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_view_location_level_xml(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					level.to_string(), idfk.to_string(), fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_location_level_remove_selected_xml(int level, string ids, bool fieldtextasbase64 = true) {
			string Retorno = "";
			string[] ValuesArray = {
				level.to_string(), "{"+ids+"}", fieldtextasbase64.to_string()
			}
			;
			//GLib.print("Llega hasta aqui 3 \n");
			var  Conexion = Postgres.connect_db (this.ConnString());
	...
	public class TableLocationLevel1:PostgreSQLConnection {
		...
		public string fun_location_level1_edit_xml_from_hashmap(HashMap<string, string> data, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					id.to_string(), name, code, ts, fieldtextasbase64.to_string()
				}
				;
	...
	public class TableIncomingCalls:PostgreSQLConnection {
		...
		public string fun_view_incomingcalls_xml(string datestart, string dateend, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					datestart.to_string(), dateend.to_string(), fieldtextasbase64.to_string()
				}
				;
	...
	public class TableContacts:PostgreSQLConnection {
		...
		public string fun_contact_change_groups_xml(int idcontact, int idgroup, bool belong, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcontact.to_string(), idgroup.to_string(), belong.to_string(), fieldtextasbase64.to_string()
				}
				;
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_contact_change_groups_xml($1::integer, $2::integer, $3::boolean, $4::boolean) AS return;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var reg in this.Result_FieldName(ref Resultado)) {
						RetornoX = reg["return"].Value;
					}
		...
		public string fun_view_contacts_groups_xml(int idcontact, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idcontact.to_string(), fieldtextasbase64.to_string()
				}
				;
				var Resultado = this.exec_params_minimal (ref Conexion, "SELECT * FROM fun_view_contacts_groups_xml($1::integer, $2::boolean) AS return;", valuesin);
				if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
					foreach(var reg in this.Result_FieldName(ref Resultado)) {
						RetornoX = reg["return"].Value;
					}
		...
		public string fun_contacts_edit_xml(int inidcontact, bool inenable, string intitle, string infirstname, string inlastname, int ingender, string inbirthday, int intypeofid, string inidentification, string inweb, string inemail1, string inemail2, int inidaddress, string innote, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					inidcontact.to_string(), inenable.to_string(), intitle, infirstname, inlastname, ingender.to_string(), inbirthday, intypeofid.to_string(), inidentification, inweb, inemail1, inemail2, inidaddress.to_string(), innote, fieldtextasbase64.to_string()
				}
				;
	...
	public class TableSMSIn:PostgreSQLConnection {
		...
		public string fun_view_smsin_table_filter_xml(string start, string end, int rows, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					start, end, rows.to_string(), fieldtextasbase64.to_string()
				}
				;
		...
		public int fun_smsin_insert(int inidport, SMS_Status instatus, Datetime indatesms, string inphone, string inmsj, string innote = "") {
			string[] valuessms = {
				inidport.to_string(), ((int)instatus).to_string(), indatesms.to_string(), inphone, inmsj, innote
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
	...
	public class TableOutgoing:PostgresuSMS {
		...
		public int log(int idsmsout, int idsim, SMSOutStatus status, int parts, int part) {
			var Retorno = 0;
			string[] valuesin = {
				idsmsout.to_string(), idsim.to_string(), ((int)status).to_string(), parts.to_string(), part.to_string()
			}
			;
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
		...
		public int fun_outgoing_new(int idowner, int inidphone, string inphone, string inmsg,  DateTime indatetosend = new DateTime.now_local(),  int inpriority = 5, int inidprovider = 0, int inidsim = 0, int inidsmstype = 0, bool inreport = false, bool inenablemsgclass = false, edwinspire.PDU.DCS_MESSAGE_CLASS inmsgclass =  edwinspire.PDU.DCS_MESSAGE_CLASS.TE_SPECIFIC, string innote = "") {
			string[] valuessms = {
				inidprovider.to_string(), inidsim.to_string(), inidsmstype.to_string(), inidphone.to_string(), inphone, inmsg, indatetosend.to_string(), inpriority.to_string(), inreport.to_string(), inenablemsgclass.to_string(), ((int)inmsgclass).to_string(), idowner.to_string(), innote
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
		...
		public string fun_outgoing_new_xml(int idowner, int inidphone, string inphone, string inmsg,  DateTime indatetosend = new DateTime.now_local(),  int inpriority = 5, int inidprovider = 0, int inidsim = 0, int inidsmstype = 0, bool inreport = false, bool inenablemsgclass = false, edwinspire.PDU.DCS_MESSAGE_CLASS inmsgclass =  edwinspire.PDU.DCS_MESSAGE_CLASS.TE_SPECIFIC, string innote = "", bool fieldtextasbase64 = true) {
			string[] valuessms = {
				inidprovider.to_string(), inidsim.to_string(), inidsmstype.to_string(), inidphone.to_string(), inphone, inmsg, indatetosend.to_string(), inpriority.to_string(), inreport.to_string(), inenablemsgclass.to_string(), ((int)inmsgclass).to_string(), idowner.to_string(), innote, fieldtextasbase64.to_string()
			}
			;
			string Retorno = "<table></table>";
			var  Conexion = Postgres.connect_db (this.ConnString());
		...
		public int fun_outgoing_new_now(int idowner, int inidphone, string inphone, string inmsg, int inpriority = 5, int inidprovider = 0, int inidsim = 0, int inidsmstype = 0, bool inreport = false, bool inenablemsgclass = false, edwinspire.PDU.DCS_MESSAGE_CLASS inmsgclass =  edwinspire.PDU.DCS_MESSAGE_CLASS.TE_SPECIFIC, string innote = "") {
			string[] valuessms = {
				inidprovider.to_string(), inidsim.to_string(), inidsmstype.to_string(), inidphone.to_string(), inphone, inmsg,  inpriority.to_string(), inreport.to_string(), inenablemsgclass.to_string(), ((int)inmsgclass).to_string(), idowner.to_string(), innote
			}
			;
			int Retorno = -1;
			var  Conexion = Postgres.connect_db (this.ConnString());
		...
		public string fun_outgoing_new_now_xml(int idowner, int inidphone, string inphone, string inmsg, int inpriority = 5, int inidprovider = 0, int inidsim = 0, int inidsmstype = 0, bool inreport = false, bool inenablemsgclass = false, edwinspire.PDU.DCS_MESSAGE_CLASS inmsgclass =  edwinspire.PDU.DCS_MESSAGE_CLASS.TE_SPECIFIC, string innote = "", bool fieldtextasbase64 = true) {
			string[] valuessms = {
				inidprovider.to_string(), inidsim.to_string(), inidsmstype.to_string(), inidphone.to_string(), inphone, inmsg,  inpriority.to_string(), inreport.to_string(), inenablemsgclass.to_string(), ((int)inmsgclass).to_string(), idowner.to_string(), innote, fieldtextasbase64.to_string()
			}
			;
			string Retorno = "<table></table>";
			var  Conexion = Postgres.connect_db (this.ConnString());
...
*/
	...
	public class ProviderTable:PostgreSQLConnection {
		...
		public string fun_provider_edit_xml(int inidprovider, bool inenable,  string inname, string innote, string ints, bool fieldtextasbase64 = true) {
			string Retorno = "";
			string[] ValuesArray = {
				inidprovider.to_string(), inenable.to_string(), inname, innote, ints, fieldtextasbase64.to_string()
			}
			;
			var  Conexion = Postgres.connect_db (this.ConnString());
			if(Conexion.get_status () == ConnectionStatus.OK) {
...
*/
		...
		public string fun_address_edit_xml(int inidaddress, int inidlocation, double ingeox, double ingeoy, string f1, string f2, string f3, string f4, string f5, string f6, string f7, string f8, string f9, string f10, string ints, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					inidaddress.to_string(), inidlocation.to_string(), ingeox.to_string(), ingeoy.to_string(), f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, ints, fieldtextasbase64.to_string()
				}
				;
		...
		public string fun_view_address_byid_xml(int idaddress, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idaddress.to_string(), fieldtextasbase64.to_string()
				}
				;
	...
	public class PhoneTable:PostgreSQLConnection {
		public string fun_view_contacts_phones_with_search_xml(string contact_phone_search, string exclude_idphones, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					contact_phone_search, "{"+exclude_idphones+"}", fieldtextasbase64.to_string()
				}
				;
...
*/
		...
		public string  fun_contact_phones_edit_xml(int idphone, int inidlocation, double ingeox, double ingeoy, string f1, string f2, string f3, string f4, string f5, string f6, string f7, string f8, string f9, string f10, string ints, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idphone.to_string(), inidlocation.to_string(), ingeox.to_string(), ingeoy.to_string(), f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, ints, fieldtextasbase64.to_string()
				}
				;
...
*/
		...
		public string fun_phones_table_xml(int inidphone, int inidcontact, bool inenable, string inphone, int intypephone, int inidprovider, string inphone_ext, int inidaddress, int inubiphone, string innote, string ints, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					inidphone.to_string(), inidcontact.to_string(), inenable.to_string(), inphone, intypephone.to_string(), inidprovider.to_string(), inphone_ext, inidaddress.to_string(), inubiphone.to_string(), innote, ints,  fieldtextasbase64.to_string()
				}
				;
		...
		public string byId_Xml(int idphone, bool fieldtextasbase64 = true) {
			...
			if(Conexion.get_status () == ConnectionStatus.OK) {
				string[] valuesin = {
					idphone.to_string(), fieldtextasbase64.to_string()
				}
				;
