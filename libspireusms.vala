...
*/
	public class Device: ModemGSM {
		//[Description(nick = "Delegado de un metodo que devuelve int", blurb = "")]
		//public delegate int method_with_sleep_delegate();
		//[Description(nick = "Delegado de un metodo que podra ser ejecutado en un hilo (threaded)", blurb = "")]
		//public delegate void* method_to_thread_delegate();
		...
		/*
public int method_with_sleep(method_with_sleep_delegate m, int usleepTime){
int r = m();
Thread.usleep(usleepTime);
return r;
}
*/
		/*
public void method_to_thread(method_to_thread_delegate p){
  if (!Thread.supported()) {
        stderr.printf("Cannot run without threads.\n");
 //       return 1;
    }else{
try{
//Thread.create<void>(p, false);
Thread.create<void*>(p, false);
}
catch(ThreadError e){
print(e.message);
...
*/
		...
		public void get_sim() {
			...
			foreach(var x in this.CPBF("usms")) {
				...
				this.SIM = DBaseSIM.fun_sim_getrow(x.Number);
				break;
			}
			if(SIM.id <= 0) {
				foreach(var x in this.CPBF("USMS")) {
